...
switch ($tipo) {
    ...
    case 'clase':
        ...
        break;
    case 'clase_cursando':
        $clase_info = $conn->query("SELECT id, edad_requerida FROM clases_regulares WHERE nombre = '$q'")->fetch_assoc();
        if ($clase_info) {
            $clase_id = intval($clase_info['id']);
            // Conquistadores que tienen esta clase asignada
            $where = "(m.tipo = 'conquistador' AND m.activo = 1 AND m.clase_actual_id = $clase_id)";
            // Líderes que enseñan esta clase
            ...
            <tbody>
                <?php while ($r = $resultados->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($r['clases'])): ?>
                                <?php foreach (explode(', ', $r['clases']) as $cl): ?>
                                    <?= badge_clase($cl) ?>
                                <?php endforeach; ?>
                            <?php else: ?>
                                -
